...
public class PostFilter implements Filter {
	...
	if (debug) {
	    log("PostFilter:DoBeforeProcessing");
	}
	...
	if("POST".equals(r.getMethod())){
		...
		text = text.replaceAll(replaceList[i], replaceList[i+1]);
    ...
    private void doAfterProcessing(ServletRequest request, ServletResponse response)
	    throws IOException, ServletException {
	if (debug) {
	    log("PostFilter:DoAfterProcessing");
	}
	...
	if (filterConfig != null) {
	    if (debug) {		
		log("PostFilter:Initializing filter");
	    }
	}
    }
	...
	StringBuffer sb = new StringBuffer("PostFilter(");
